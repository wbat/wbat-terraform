...
- When AWS is available the credential is currently an **admin key** (the operator's own), not a scoped agent identity. Nothing in IAM stops a write, a delete, or a secret read, so the restraint has to come from you: prefer read-only calls, and treat anything that mutates infrastructure as requiring a PR and an HCP Terraform apply even though the key would let you skip that. CloudTrail cannot distinguish your calls from the operator's, so unexplained activity is expensive to attribute.
- Shell access is `aws ssm start-session`, **not SSH**. It lands as `ssm-user` with sudo on a box serving ~91 production sites — treat it as production, and prefer read-only diagnosis.
