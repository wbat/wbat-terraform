...
| Layer | Version |
|-------|---------|
| HCP Terraform workspaces | **~> 1.15.0** (after step 4) |
...
3. **HCP Terraform ~> 1.15.0** — Set `local.terraform_version = "~> 1.15.0"` in `tfc/locals.tf` and apply via `wbat-terraform-tfc`. HCP picks the newest 1.15.x release automatically.
